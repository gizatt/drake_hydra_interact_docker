...
RUN wget https://drake-packages.csail.mit.edu/drake/nightly/drake-latest-focal.tar.gz -P /opt \
    && tar -xzf /opt/drake-latest-focal.tar.gz -C /opt/ \
    && rm -f /opt/drake-latest-focal.tar.gz
...
RUN  apt-get update \
  && apt-get install -y git tmux build-essential python3-pip \
  && rm -rf /var/lib/apt/lists/*
